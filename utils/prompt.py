...
def make_prompt_for_evaluation(examples: List[Dict[str, Any]], target_news: Dict[str, Any], max_tokens: int = 8192) -> str:
    evaluation_criteria = """You are an expert news editor evaluating a draft article generated from a summary, using a human-written news article as the gold standard.

The draft article has experienced a drop in quality metrics compared to previous versions. Your task is to provide HIGHLY TARGETED, actionable feedback to the writer for the next iteration.

**CRITICAL RULES FOR FEEDBACK:**
1. **Factuality Limit:** The writer ONLY has access to their original short summary. You MUST NOT suggest adding new entities, background context, specific numbers, or quotes that are not already present.
2. **Focus Areas:** Evaluate the draft based on:
   - **Conciseness & Flow:** Are the sentences natural, or repetitive and robotic?
   - **Closeness to Human Style:** Does it sound like a professional news report (Title, Lead, Body sequence)?
3. **Target Specific Sentences (CRUCIAL):** For every suggestion, you MUST explicitly locate the exact sentence or paragraph that needs modification (e.g., "The second sentence in the first paragraph: '...' is too repetitive."). Do NOT give vague, general advice.

Based on the differences between the human news and the draft, provide a MAXIMUM OF 2 specific, concise revision suggestions. You MUST pinpoint the exact sentences causing the issues. Focus on stylistic phrasing, sentence connection, or structural reorganization, NOT adding facts."""
    ...
    target_format = "========== TARGET NEWS (YOU NEED TO EVALUATE THIS) ==========\nNews summary:\nHuman-written news:\nLLM news:\n"
    ...
    prompt += "========== REFERENCE EXAMPLES (READ ONLY - DO NOT EVALUATE) ==========\n"
    ...
    prompt += "========== TARGET NEWS (YOU NEED TO EVALUATE THIS) ==========\n"
    ...
    prompt += f"{target_llm_type} (Draft to evaluate): {target_llm}\n\n" 

    prompt += "You need to ONLY provide revision suggestions for the ONE article under 'TARGET NEWS'. DO NOT evaluate or comment on the reference examples. Output your evaluation directly."
...
def _latest_completed_iteration_for_news(per_news_evaluator: PerNewsEvaluation, news_id: str, max_iter: int) -> Optional[int]:
    ...
    for it in range(max_iter, 0, -1):
        m = per_news_evaluator.get_iteration_metrics(news_id, it)
...
- Vocabulary choices that match reference texts
- Sentence structures and sequence patterns
- Overall similarity and style
...
"""
    ...
    inst2 = f"""**TASK:** Write a professional news article based STRICTLY on the new summary provided below.

**GUIDELINES:**
1. **Length & Style:** About {low_words}–{high_words} words. Emulate the human-like flow and phrasing of the examples. Use natural journalistic transitions to expand the text if needed, rather than listing bare facts.
2. **Factual Strictness:** The summary is your ONLY source of facts. You MUST NOT introduce new entities, numbers, dates, quotes, or background information. Paraphrasing is encouraged, but the core meaning must remain identical to the summary.
3. **Format:** Structure it naturally with a clear Headline, followed by the news body.

**OUTPUT RULES:**
- Output ONLY the raw news article text starting directly with the headline. 
- DO NOT include introductory phrases (e.g., "Here is...", "Title:").
- DO NOT add web artifacts at the end (e.g., "Source:", "Read more").
